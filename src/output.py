...
def print_banner(args):
    ...
    if not args.lang:
        rows.append(
            f"- {cl.YELLOW}Выбор языка:{cl.BLUE} `--lang en` / `--lang ru`{cl.RESET}"
        )
    if not args.threads:
        rows.append(
        ...
        rows.append(
            f"- {cl.BLUE}{t('Instead of high `-t` value consider using')} {cl.YELLOW}`--copies 2`{cl.RESET}"
        )
    ...
    rows.append(
        f"- {cl.YELLOW}{t('Мы не наносим ущерб инфраструктуре городов, морали и нравственности мирного населения')}-{cl.CYAN} - ALYONA.Z RUSSIAN CYBER ARMY TEAM"
    )
